...
<table id="ghatable" class="display table table-bordered table-stripe" cellspacing="0" width="100%">
     ...
     <tbody>
          <?php
          $objActaprendizaje = new Actaprendizaje();
          $actproyectos = $objActaprendizaje->actaprendizaje();
          if(sizeof($actproyectos) > 0){
               foreach ($actproyectos as $row){
                    ?>
                    <tr>
                         <td><?php echo $row['cod_acti'] ?></td>
                         <td><?php echo $row['nom_acti'] ?></td>
                         <td>
                              <a href="update.php?u=<?php echo $row['cod_acti'] ?>"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> Editar</a>
                         </td>
                         <td>
                              <a onclick="return confirm('Desea eliminar el registro')" href="delete.php?d=<?php echo $row['cod_acti'] ?>"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Eliminar</a>
                         </td>
                    </tr>
                    <?php
               }
